...
static ssize_t fingerprint_read(struct file *file, char __user *buf, size_t count, loff_t *off);
static ssize_t fingerprint_write(struct file *f, const char __user *buf, size_t cnt, loff_t *off);
static int fingerprint_flush(struct file *file, fl_owner_t owner);
...
int fingerprint_flush(struct file *file, fl_owner_t owner) {
    printk("Flush");
    return 0;
}
...
static int ep_1_close_fingerprint(struct usb_device *usbDevice){
    ...
    ep_1_data[2] = 0x02;
    ret = usb_bulk_msg(usbDevice, usb_sndbulkpipe(usbDevice, 0x1), &ep_1_data, 3, &actual_length, 0);
    if (ret) {
        printk(KERN_ERR "Bulk message returned %d\n", ret);
        printk("actual length : %d\n", actual_length);
    }
